...
Egg::Egg(int size, int type) {
  ...
  texturePoints = new vector2*[size];
  for (int i = 0; i < size; i++) {
    points[i] = new vector3[size];
    normals[i] = new vector3[size];
    texturePoints[i] = new vector2[size];
  }
  generateEgg();
  generateNormals();
}
...
void Egg::generateEgg() {
  ...
  for (int u = 0; u < N; u++) {
    for (int v = 0; v < N; v++) {
      ...
      x = (-90 * pow(uf, 5) + 225 * pow(uf, 4) - 270 * pow(uf, 3) +
           180 * pow(uf, 2) - (45 * static_cast<double>(uf))) *
          cos(M_PI * vf);
      y = 160 * pow(uf, 4) - 320 * pow(uf, 3) + 160 * pow(uf, 2);
      z = (-90 * pow(uf, 5) + 225 * pow(uf, 4) - 270 * pow(uf, 3) +
           180 * pow(uf, 2) - (45 * static_cast<double>(uf))) *
          sin(M_PI * vf);
      points[u][v][0] = x;
      points[u][v][1] = y;
      points[u][v][2] = z;
      texturePoints[u][v][0] = uf;
      texturePoints[u][v][1] = vf;
      if (y > height) {
        height = y;
      }
    }
...
void Egg::generateNormals() {
  ...
  for (int u = 0; u < N; u++) {
    for (int v = 0; v < N; v++) {
      uf = static_cast<float>(u) / (static_cast<float>(N) - 1);
      vf = static_cast<float>(v) / (static_cast<float>(N) - 1);
      xu = (-450 * pow(uf, 4) + 900 * pow(uf, 3) - 810 * pow(uf, 2) +
            (360 * static_cast<double>(uf)) - 45) *
           cos(M_PI * vf);
      xv = M_PI *
           (90 * pow(uf, 5) - 225 * pow(uf, 4) + 270 * pow(uf, 3) -
            180 * pow(uf, 2) + (45 * static_cast<double>(uf))) *
           sin(M_PI * vf);
      yu = 640 * pow(uf, 3) - 960 * pow(uf, 2) + 320 * static_cast<double>(uf);
      zu = (-450 * pow(uf, 4) + 900 * pow(uf, 3) - 810 * pow(uf, 2) +
            (360 * static_cast<double>(uf)) - 45) *
           sin(M_PI * vf);
      zv = -M_PI *
           (90 * pow(uf, 5) - 225 * pow(uf, 4) + 270 * pow(uf, 3) -
...
void Egg::eggDrawLines() {
  ...
  for (int u = 0; u < size; u++) {
    for (int v = 0; v < size; v++) {
      if (v != size - 1) {
        glVertex3f(points[u][v + 1][0], points[u][v + 1][1],
                   points[u][v + 1][2]);
      } else {
  ...
  for (int u = 0; u < size; u++) {
    for (int v = 0; v < size; v++) {
      if (u != N - 1) {
        glVertex3f(points[u + 1][v][0], points[u + 1][v][1],
                   points[u + 1][v][2]);
      } else {
...
void Egg::eggDrawTriangles() {
  ...
  for (int u = 0; u < N; u++) {
    for (int v = 0; v < N; v++) {
      ...
      glTexCoord2f(texturePoints[u][(v + 1 - N * vdiff)][0],
                 texturePoints[u][(v + 1 - N * vdiff)][1]);
      ...
      glTexCoord2f(texturePoints[(u + 1 - N * udiff)][v][0],
                 texturePoints[(u + 1 - N * udiff)][v][1]);
      glVertex3f(points[(u + 1 - N * udiff)][v][0],
                 points[(u + 1 - N * udiff)][v][1],
                 points[(u + 1 - N * udiff)][v][2]);

      glNormal3f(normals[u][v][0], normals[u][v][1], normals[u][v][2]);
      glTexCoord2f(texturePoints[u][v][0], texturePoints[u][v][1]);
      glVertex3f(points[u][v][0], points[u][v][1], points[u][v][2]);
      glEnd();
      ...
      glTexCoord2f(texturePoints[(u + 1 - N * udiff)][(v + 1 - N * vdiff)][0],
                   texturePoints[(u + 1 - N * udiff)][(v + 1 - N * vdiff)][1]);
